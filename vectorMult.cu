...
__device__ inline cmplx_half2 operator*(const cmplx_half2& a, const cmplx_half2& b) {
    #if COMPCAP >= 530
        const auto temp1 = __hmul2(a.x, b.x);
        const auto temp2 = __lowhigh2highlow(b.x);
        const auto temp3 = __hmul2(a.x, temp2);
        
        //method 1
//        const auto ntemp1 = __hneg2(temp1); // - ac | -bd
//        const auto temp4 = __highs2half2(ntemp1, temp3); // - bd | bc
//        const auto temp5 = __lows2half2(temp1, temp3); //  ac | ad 
//        const  cmplx_half2 answer = {__hadd2(temp4, temp5)}; // ac - bd | bc + ad
        
        //method 2
        const auto re = __hsub(__low2half(temp1), __high2half(temp1)); // ac - bd
        const auto im = __hadd(__low2half(temp3), __high2half(temp3));// ad + bc
        const cmplx_half2 answer = {__halves2half2(re, im)};
    #else
        //do nothing
    #endif
        return answer;
